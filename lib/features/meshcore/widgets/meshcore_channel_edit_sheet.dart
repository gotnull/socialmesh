...
//
// PSK input modes:
//   - paste hex (32 hex chars = 16 raw bytes)                           [D31]
//   - paste full channel code (`name:pskHex`) — the share/import        [D31]
//     format already used by D29's QR share/import. The "Paste from
//     channel code" button surfaces this without forcing the user to
//     split it manually.
//   - dice / random key generator (16 cryptographically-random bytes)  [D34d]
//   - derive from a passphrase (HMAC-SHA256 under SocialMesh-internal  [D34d]
//     label — see `lib/services/meshcore/channel_psk_kdf.dart`)
//
...
import '../../../services/meshcore/channel_psk_kdf.dart';
...
class _MeshCoreChannelEditSheetState
  ...
  /// D34d: fill the PSK field with a fresh 16-byte cryptographically-random
  /// hex string. Mirrors the dice-icon affordance in the upstream reference;
  /// no logging of the generated bytes.
  void _generateRandomPsk() {
    final hex = formatPskHex(randomPsk());
    setState(() {
      _pskController.text = hex;
    });
    HapticFeedback.lightImpact();
    showSuccessSnackBar(
      context,
      context.l10n.meshcoreChannelEditRandomPskGenerated,
    );
  }

  /// D34d: open a small bottom sheet that takes a UTF-8 passphrase and
  /// derives a 16-byte PSK via the SocialMesh-internal HMAC-SHA256 KDF.
  /// On submit the PSK field is filled with the 32-char hex form; the
  /// passphrase itself is never persisted, never logged, and stays scoped
  /// to the sheet's local state.
  Future<void> _openPassphraseSheet() async {
    HapticFeedback.lightImpact();
    final derived = await AppBottomSheet.showScrollable<Uint8List>(
      context: context,
      initialChildSize: 0.6,
      minChildSize: 0.4,
      maxChildSize: 0.95,
      builder: (controller) =>
          _PassphraseDeriveSheet(scrollController: controller),
    );
    if (!mounted || derived == null) return;
    final hex = formatPskHex(derived);
    safeSetState(() {
      _pskController.text = hex;
    });
    showSuccessSnackBar(
      context,
      context.l10n.meshcoreChannelEditPassphrasePskGenerated,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Sheet shell is `context.background` globally (see
    // `AppBottomSheet`); nested FieldGroupCard / SettingsTile
    // (`context.card`, lighter) pop naturally on top.
    return Form(
      key: _formKey,
      child: ListView(
...
/// D34d passphrase-derive sheet. Returns the derived 16-byte PSK on
/// confirm or `null` on cancel. The passphrase value never leaves the
/// `_passphraseController` — the parent sheet receives bytes only.
class _PassphraseDeriveSheet extends ConsumerStatefulWidget {
  final ScrollController scrollController;

  const _PassphraseDeriveSheet({required this.scrollController});

  @override
  ConsumerState<_PassphraseDeriveSheet> createState() =>
      _PassphraseDeriveSheetState();
}

class _PassphraseDeriveSheetState extends ConsumerState<_PassphraseDeriveSheet>
    with LifecycleSafeMixin {
  final _formKey = GlobalKey<FormState>();
  ...
  String? _validate(String? value, AppLocalizations l10n) {
    if ((value ?? '').trim().isEmpty) {
      return l10n.meshcoreChannelEditPassphraseRequired;
    }
    return null;
  }
  ...
  void _onGenerate() {
    ...
    final psk = derivePskFromPassphrase(passphrase);
    HapticFeedback.lightImpact();
    Navigator.of(context).pop(psk);
  }
        ...
        child: ListView(
          ...
          children: [
            Padding(
              ...
              child: Text(
                l10n.meshcoreChannelEditPassphraseSheetTitle,
                style: TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.w600,
                  color: context.textPrimary,
            ...
            Padding(
              ...
              child: Text(
                l10n.meshcoreChannelEditPassphraseSheetHint,
                style: TextStyle(fontSize: 13, color: context.textTertiary),
              ),
            ),
            FieldGroupCard(
              child: TextFormField(
                key: const ValueKey('meshcore-channel-edit-passphrase-field'),
                ...
                decoration: InputDecoration(
                  labelText: l10n.meshcoreChannelEditPassphraseLabel,
                  labelStyle: TextStyle(color: context.textSecondary),
                  hintText: l10n.meshcoreChannelEditPassphraseHint,
                  hintStyle: TextStyle(color: SemanticColors.muted),
                  helperText: l10n.meshcoreChannelEditPassphraseHelper,
                  helperStyle: TextStyle(
                    color: context.textTertiary,
                    fontSize: 12,
                  ),
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: PrimaryGradientButton(
                      key: const ValueKey(
                        'meshcore-channel-edit-passphrase-submit',
                      ),
                      label: l10n.meshcoreChannelEditPassphraseGenerate,
                      icon: Icons.password_rounded,
                      accentColor: accent,
